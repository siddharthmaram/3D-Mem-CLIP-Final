...
def main(cfg, start_ratio=0.0, end_ratio=1.0, split=1):
    ...
    for scene_data_file in scene_data_list:
        ...
        with gzip.open(os.path.join(cfg.test_data_dir, scene_data_file), "rt", encoding="utf-8") as f:
            num_episode += len(json.load(f)["episodes"])
    logging.info(
    ...
    for scene_data_file in scene_data_list:
        ...
        # )
        scene_data = json.load(
            gzip.open(os.path.join(cfg.test_data_dir, scene_data_file), "rt", encoding="utf-8")
        )
        ...
        for episode_idx, episode in enumerate(scene_data["episodes"]):
            ...
            for subtask_idx, (goal_type, subtask_goal) in enumerate(
                ...
                # In case of inf
                if math.isinf(agent_subtask_distance) or math.isnan(agent_subtask_distance):
                    success_by_distance = False
                    logging.warning(
                        f"Fail: Target unreachable due to NavMesh/Dataset error. Distance returned {agent_subtask_distance}!"
                    )
                    # Sanitize the variable to prevent 'NaN' contagion in downstream SPL math.
                    # Setting it to 0.0 or a heavily penalized large number ensures SPL evaluates to 0 
                    # rather than breaking the entire test suite.
                    agent_subtask_distance = 0.0
                elif agent_subtask_distance < cfg.success_distance:
                ...
                overall_success = success_by_distance

                if cfg.save_video:
                    logger.save_video(subtask_id, subtask_metadata, overall_success)
