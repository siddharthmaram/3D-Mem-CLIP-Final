...
class Scene:
    ...
    def move_agent_to_goal_room(self, goal_pos: np.ndarray) -> Optional[np.ndarray]:
        """
        Teleport agent to a random navigable point in the same semantic region
        as the goal. Uses robust heuristics instead of strict AABB checks.
        """
        ...
        if goal_pos is None:
            logging.warning("Goal position is not on the navmesh.")
            return None

        goal_pos = np.array(goal_pos)

        regions = self.simulator.semantic_scene.regions
        if not regions:
            logging.warning("No semantic regions found.")
            return None
        ...
        for region in regions:
            ...
            center = np.array(region.aabb.center)
        ...
        if goal_region is None:
            logging.warning("Could not find a valid goal region.")
            return None

        region_center = np.array(goal_region.aabb.center)

        max_attempts = 200
        radius = 5  # tune based on room size

        for _ in range(max_attempts):
            sampled_pt = pathfinder.get_random_navigable_point_near(
                region_center,
                radius
            )

            if sampled_pt is None:
                continue

            sampled_pt = np.array(sampled_pt)

            # Optional: ensure it's not too far from goal (extra safety)
            # if np.linalg.norm(sampled_pt - goal_pos) > 5.0:
            #     continue

            agent_state = self.agent.get_state()
            agent_state.position = sampled_pt
            self.agent.set_state(agent_state)

            logging.info(f"Teleported to region {goal_region.id}")
            return sampled_pt

        logging.warning("Falling back to sampling near goal directly.")

        fallback_pt = pathfinder.get_random_navigable_point_near(
            goal_pos,
            radius=3
        )
        ...
        if fallback_pt is not None:
            agent_state = self.agent.get_state()
            agent_state.position = fallback_pt
            self.agent.set_state(agent_state)

            return np.array(fallback_pt)

        logging.error("Failed to find any valid teleport point.")
        return None
