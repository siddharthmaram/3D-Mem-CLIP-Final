...
from src.model_utils import extract_core_goal
...
class Logger:
    ...
    def log_subtask_result(
        ...
        if math.isnan(gt_subtask_explore_dist) or math.isinf(gt_subtask_explore_dist):
            self.spl_by_snapshot[subtask_id] = 0.0
            self.spl_by_distance[subtask_id] = 0.0
        else:
    ...
    def log_frame(self, rgb_frame):
        """
        Stores the current RGB observation. 
        rgb_frame should be a numpy array (e.g., obs["rgb"] or obs["color_sensor"])
        """
        if rgb_frame.shape[-1] == 4:
            rgb_frame = rgb_frame[..., :3]
            
        self.subtask_frames.append(rgb_frame)
    ...
    def save_video(self, subtask_id, subtask_metadata, is_success):
        ...
        assert self.output_dir is not None, "Episode directory not initialized."
